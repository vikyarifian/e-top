...
func Layout(title string, user dto.UserAuth, content ...templ.Component) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 2, " - etop</title><link rel=\"icon\" href=\"data:,\"><link rel=\"stylesheet\" href=\"/public/css/styles.css\"><script src=\"/public/js/htmx.min.js\"></script><script src=\"/public/js/cdn.min.js\" defer></script><script src=\"/public/js/lucide.js\"></script><style>\r\n\t\t\t\t\r\n\t\t\t</style></head><body class=\"min-h-screen bg-background text-foreground\" x-data=\"{ \r\n\t\t\t\tsidebarOpen: false, \r\n\t\t\t\tsidebarCollapsed: false, \r\n\t\t\t\tdarkMode: (localStorage.getItem('darkMode') === 'true' ? true : false),\r\n\t\t\t\tisMobile: window.innerWidth < 768\r\n\t\t\t}\" x-init=\"\r\n\t\t\t\t$watch('darkMode', val => {\r\n\t\t\t\t\tlocalStorage.setItem('darkMode', val);\r\n\t\t\t\t\tif (val) {\r\n\t\t\t\t\t\tdocument.documentElement.classList.add('dark');\r\n\t\t\t\t\t} else {\r\n\t\t\t\t\t\tdocument.documentElement.classList.remove('dark');\r\n\t\t\t\t\t}\r\n\t\t\t\t});\r\n\t\t\t\tif (darkMode) {\r\n\t\t\t\t\tdocument.documentElement.classList.add('dark');\r\n\t\t\t\t}\r\n\t\t\t\twindow.addEventListener('resize', () => {\r\n\t\t\t\t\tisMobile = window.innerWidth < 768;\r\n\t\t\t\t\tif (!isMobile) sidebarOpen = false;\r\n\t\t\t\t});\r\n\t\t\t\"><div class=\"flex h-screen\"><!-- Mobile Sidebar Overlay --><div x-show=\"sidebarOpen && isMobile\" x-transition:enter=\"transition-opacity ease-linear duration-300\" x-transition:enter-start=\"opacity-0\" x-transition:enter-end=\"opacity-100\" x-transition:leave=\"transition-opacity ease-linear duration-300\" x-transition:leave-start=\"opacity-100\" x-transition:leave-end=\"opacity-0\" class=\"fixed inset-0 z-40 bg-black bg-opacity-50 md:hidden\" @click=\"sidebarOpen = false\"></div><!-- Mobile Sidebar --><div x-show=\"sidebarOpen && isMobile\" x-transition:enter=\"transition ease-in-out duration-300 transform\" x-transition:enter-start=\"-translate-x-full\" x-transition:enter-end=\"translate-x-0\" x-transition:leave=\"transition ease-in-out duration-300 transform\" x-transition:leave-start=\"translate-x-0\" x-transition:leave-end=\"-translate-x-full\" class=\"fixed inset-y-0 left-0 z-50 w-64 bg-card border-r border-border md:hidden\">")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 3, "</div><!-- Desktop Sidebar --><div class=\"hidden md:flex flex-col bg-card border-r border-border transition-all duration-300 ease-in-out\" :class=\"sidebarCollapsed ? 'w-16' : 'w-64'\">")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		templ_7745c5c3_Err = Sidebar().Render(ctx, templ_7745c5c3_Buffer)
